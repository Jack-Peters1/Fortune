...
def handle_client(conn, addr):
    ...
    with clients_lock:
        ...
        client_ids.append(conn.getpeername()[0])
    print(client_ids)
    with clients_lock:
    ...
    while connected:  # While each connection is established...
        target = conn.recv(HEADER).decode(FORMAT)  # Begin searching for messages. The first message, target, is for the ID that the server will try to PM.
        print(target)
            ...
            with clients_lock:  # For all of the clients in all threads...
                for client in clients:
                    if msg != DISCONNECT_MESSAGE:  # If the client is not trying to disconnect...
                        ...
                        if client_ids.index(conn.getpeername()[0]) == target:  # And also Send the PM to the target and only to the target.
